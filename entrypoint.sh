...
# 启动 WebSocket 消息推送服务（ipadWs.jar）
WS_JAR_PATH="/app/ws/ipadWs.jar"
if [ -f "$WS_JAR_PATH" ]; then
    echo "启动 WebSocket 消息推送服务: $WS_JAR_PATH"
    nohup java -jar "$WS_JAR_PATH" > /app/logs/ipadWs.log 2>&1 &
    echo "ipadWs.jar 已在后台启动"
else
    echo "未找到 $WS_JAR_PATH，跳过启动 WebSocket 服务"
fi
